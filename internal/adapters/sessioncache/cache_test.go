...
func TestCache_FullSync_IncludesMetadataOnlySessions(t *testing.T) {
	tempDir := t.TempDir()
...
`,
	...
	if got := byID["11111111-1111-1111-1111-111111111111"].MessageCount; got != 2 {
		t.Fatalf("conversation message_count = %d, want 2", got)
	}
	...
	systemOnly := byID["33333333-3333-3333-3333-333333333333"]
	if got := systemOnly.Summary; got != "33333333" {
		t.Fatalf("system-only summary = %q, want short session id", got)
	}
	if got := systemOnly.Branch; got != "main_livechat" {
		t.Fatalf("system-only branch = %q, want main_livechat", got)
	}

	if got := byID["44444444-4444-4444-4444-444444444444"].Summary; got != "(session)" {
		t.Fatalf("progress-only summary = %q, want (session)", got)
	}
}
